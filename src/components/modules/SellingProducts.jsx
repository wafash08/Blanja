			...
			<div className='w-[100%] bg-white px-6 py-8  rounded-[4px]'>
				...
				<div className='w-[348px] mb-10'>
					<fieldset className='border-none flex'>
						<legend className='font-metropolis font-medium text-[14px] text-[#9B9B9B] mb-5'>
							Condition
						</legend>
				...
				<div className='w-[348px] mb-10'>
					<p className='font-metropolis font-medium text-[14px] text-[#9B9B9B] mb-5'>
						Category
					</p>
					<div className='w-full flex flex-wrap justify-start gap-4'>
						{listCategories.map((category, index) => (
							<button
								onClick={() =>
									setFormProduct({ ...formProduct, category_id: category.id })
								}
								className={`${
									formProduct.category_id === category.id
										? 'h-[30px] w-[70px] outline-none bg-[#DB3022] font-metropolis font-normal text-[14px] text-[#FFFFFF]'
										: 'h-[30px] w-[70px] outline outline-1 outline-[#9B9B9B] font-metropolis font-normal text-[14px] text-[#9B9B9B]'
								}`}
								key={index}
							>
								{category.name}
				...
				<div className='w-[348px] mb-10'>
					<div className='flex gap-5 justify-start items-end'>
						<div className='w-[70%]'>
							<InputField
								...
								name='color'
								label='Add Color'
								placeholder='Input Hex Color. Ex: #DB3022'
								onChange={e => setInputColor(e.target.value)}
							/>
						</div>
						<div className='w-[20%]'>
							<button
								onClick={handleStoreColor}
								className='w-full h-[40px] rounded-r-[25px] rounded-l-[25px] bg-[#DB3022] font-normal text-[14px] text-white border-none hover:cursor-pointer hover:bg-red-900'
							>
								Add
							</button>
						</div>
					...
					<div className='flex flex-wrap justify-start gap-4 mt-5'>
						{formProduct.colors.length !== 0 &&
							formProduct.colors.map((color, index) => (
								<button
									key={index}
									onClick={() => handleRemoveColor(index)}
									style={{ '--color-product': `${color.value}` }}
									className={`group bg-[var(--color-product)] rounded-full w-[50px] h-[50px] shadow-lg`}
								>
									<CloseMark className='w-full h-full rounded-full outline-none border-none opacity-0 group-hover:text-black group-hover:opacity-100 group-hover:bg-gray-400 group-hover:bg-opacity-30' />
								</button>
							))}
					</div>
				</div>
					...
					<div className='flex gap-5 justify-start items-end'>
						<div className='w-[70%]'>
							<InputField
								...
								name='size'
								label='Add Size'
								placeholder='Input Size. Ex: XL or Ex: 42'
								onChange={e => setInputSize(e.target.value)}
							/>
						</div>
						<div className='w-[20%]'>
							<button
								onClick={handleStoreSize}
								className='w-full h-[40px] rounded-r-[25px] rounded-l-[25px] bg-[#DB3022] font-normal text-[14px] text-white border-none hover:cursor-pointer hover:bg-red-900'
							>
								Add
							</button>
						...
						{formProduct.sizes.length !== 0 &&
							formProduct.sizes.map((size, index) => (
								<button
									...
									className={
										'group h-[30px] w-[70px] outline outline-1 outline-[#9B9B9B] font-metropolis font-normal text-[14px] text-[#9B9B9B] hover:flex hover:justify-start hover:items-center hover:pl-2 hover:pr-2 relative'
									}
								>
									{size.value}{' '}
									<CloseMark className='w-[14px] h-[14px] text-black ml-auto rounded-full outline outline-1 outline-black opacity-0 absolute group-hover:static group-hover:opacity-100  shadow-md' />
								</button>
							))}
					</div>
				</div>
							...
							<ul className='flex items-center gap-5 whitespace-nowrap'>
								{formProduct.images.map((image, index) => (
									<li
										...
										className={`relative bg-[image:var(--image-product)] bg-cover bg-center rounded-md whitespace-nowrap ${
											index === 0
												? 'w-[190px] h-[190px]'
												: 'w-[120px] h-[120px]'
										}`}
									>
										<button
											...
											className='absolute top-2 right-2 w-[24px] h-[24px] rounded-full bg-white'
										>
											<CloseMark />
										</button>
									</li>
